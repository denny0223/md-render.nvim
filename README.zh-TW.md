...
浮動、分頁或原地切換的文件預覽，會自動設定以下緩衝區專用按鍵：
...
完成 [Snacks 後端](#選用-snacks-圖片後端)設定並安裝 ImageMagick（`magick`）後，在圖片或其標題上按 Enter，即可開啟獨立圖片分頁。下列按鍵會自動設定在該分頁內，不需要額外的外掛或按鍵設定。

| 圖片分頁中的按鍵 | 動作 |
|---|---|
| `+` / `=` / `-` | 放大／縮小，範圍為完整圖片適配大小的 1～16 倍 |
| `h/j/k/l` 或方向鍵 | 每次平移目前可見圖片寬度或高度的 1/8 |
| `zH` / `zL` | 向左／右平移半個畫面 |
| `<C-d>` / `<C-u>` | 向下／上平移半個畫面 |
| `<C-f>` / `<C-b>` | 向下／上翻頁，空間允許時保留兩個畫面行的重疊範圍 |
| `gg` / `G` | 移到頂端／底端 |
| `0` / `^` | 移到最左端 |
| `$` | 移到最右端 |
| `f` | 將完整圖片放入視窗，並置中顯示 |
