...
4. Use `:MdRender toggle` to return to the source and test terminal Shift-drag selection; toggle back and verify that the image backend is retained and search state is unchanged. Separately switch among `native`, `off` and `image`; verify the reading passage, cursor character and search state. In `off`, test selection of rendered text and OSC 8 modifier-click destinations. The source buffer has no preview-generated OSC 8 links; direct terminal interaction over images is not supported.
